...
export default function Home(){
  ...
  useEffect(()=>{
    const lang=new URLSearchParams(window.location.search).get("lang") as SiteLanguage|null;
    if(lang && (uiLanguages as readonly string[]).includes(lang)) setLanguage(lang);
  },[]);
  ...
  const output=useMemo(()=>{
    ...
    if((additionalLanguages as readonly string[]).includes(language)) return additionalDraft(language as AdditionalLanguage,route,tone,{issue,impact,love,playerType,years,platform});
  ...
  async function copyDraft(text=output){
    try{await navigator.clipboard.writeText(text);setCopied(true)}catch{
      const el=document.getElementById("draft-text");if(el){const range=document.createRange();range.selectNodeContents(el);const sel=window.getSelection();sel?.removeAllRanges();sel?.addRange(range)}
    }
  }
  ...
  return <main className="wrap" dir={language==="العربية"?"rtl":"ltr"}>
    ...
    <section className="panel"><div className="stephead"><span className="stepnum">2</span><div><h2>{copy.issueTitle}</h2><p>{copy.issueHint}</p></div></div><div className="quick-controls"><label>{copy.biggestIssue}<select value={issue} onChange={e=>setIssue(e.target.value)}>{en.issues.map(i=><option value={i} key={i}>{copy.issues[i]||i}</option>)}</select></label><label>{copy.tone}<select value={tone} onChange={e=>{setTone(e.target.value as Tone);setGenerated(false)}}><option value="calm">{copy.tones.calm}</option><option value="firm">{copy.tones.firm}</option><option value="emotional">{copy.tones.emotional}</option></select></label>{route==="public"&&<label>{copy.platform}<select value={platform} onChange={e=>setPlatform(e.target.value as Platform)}>{en.platforms.map(p=><option key={p}>{p}</option>)}</select></label>}</div>
      <details className="personalise"><summary>{copy.personalTitle} <span>{copy.personalOptional}</span></summary><div className="personal-grid"><label>{copy.playerType}<select value={playerType} onChange={e=>setPlayerType(e.target.value as PlayerType)}><option value="F2P">F2P</option><option value="low spender">low spender</option><option value="moderate spender">moderate spender</option><option value="heavy spender">heavy spender</option></select></label><label>{copy.yearsPlayed}<input value={years} placeholder={copy.yearsPlaceholder} onChange={e=>setYears(e.target.value)}/></label><label className="wide">{copy.keepsPlaying}<textarea value={love} onChange={e=>setLove(e.target.value)}/></label><label className="wide">{copy.impact}<textarea value={impact} onChange={e=>setImpact(e.target.value)}/></label></div></details>
